...
public class CommercialRouteDAO extends BaseDAO<CommercialRoute> {

    private static CommercialRouteDAO instance;

    public static CommercialRouteDAO getInstance() {
        if (instance == null)
            instance = new CommercialRouteDAO();
        return instance;
    }

    @SuppressWarnings("unchecked")
    public List<CommercialRoute> getAll(Integer numElements, Integer offset)
            throws ConfigurationException, DatabaseException {
        List<CommercialRoute> result = new ArrayList();
        Cursor cursor = null;

        Object obj = new CommercialRoute();
        try {
            String query = JDBCQueryMaker.extractTablesAndColumnsForQueryObject(obj, Restriction.VOID, false);
            query = query + " ORDER BY sequence, code";
